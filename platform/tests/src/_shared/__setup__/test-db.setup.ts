...
export async function startTestDb(): Promise<DynamoDBClient> {
  ...
  if (!process.env.TABLE_NAME && !process.env.DYNAMODB_CORE_TABLE_NAME) {
    process.env.TABLE_NAME = "unifora-core-test";
    process.env.DYNAMODB_CORE_TABLE_NAME = "unifora-core-test";
  }
  if (!process.env.BASE_TABLE_NAME && !process.env.DYNAMODB_BASE_TABLE_NAME) {
    process.env.BASE_TABLE_NAME = "unifora-base-test";
    process.env.DYNAMODB_BASE_TABLE_NAME = "unifora-base-test";
  }
  if (!process.env.ANALYTICS_TABLE_NAME && !process.env.DYNAMODB_ANALYTICS_TABLE_NAME) {
    process.env.ANALYTICS_TABLE_NAME = "unifora-analytics-test";
    process.env.DYNAMODB_ANALYTICS_TABLE_NAME = "unifora-analytics-test";
  }
